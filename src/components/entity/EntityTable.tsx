...
export function EntityTable() {
  ...
  const columnC = [{ field: "id", hide: true }, ...getColumns(sequenceA, fieldC)];
      ...
      (dataObj: any) => sequenceA.reduce(
        (acc, fieldId) => {
          ...
          return {
            ...acc,
            id: dataObj.id,
            [fieldId]: fieldO.get ? fieldO.get(dataObj) : dataObj[fieldId],
          };
        },
        {},
      ),
    ),
    [dataC, fieldC, sequenceA],
  );
  ...
  function handleCellClick(cellO: CellParams) {
    ...
    if (cellO.field === columnC[1].field) {
      console.log("cellO.data.id:", cellO.data.id);

      goTo([, "update", Number(cellO.data.id)]);
    }
  }
  return (
